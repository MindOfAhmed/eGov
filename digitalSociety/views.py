...
def passport_info_validation(request):
    ...
    if serializer.is_valid():
        ...
        try:
            ...
            if (datetime.now().date() - data['issue_date']) < timedelta(days=1095):
            ...
            if is_valid:
                ...
                if RenewalRequests.objects.get(citizen=passport.citizen, request_type="Passport").exists() == False: # copilot helped correct this 
            ...
            return Response({"message": "The passport does not exist."}, status=status.HTTP_400_BAD_REQUEST)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)